...
pygame.display.set_caption("Game of Pool")
...
cue_image = pygame.image.load("assets/cuestick.png").convert_alpha()
...
# pivot joint for friction
    ...
    pivot.max_bias = 0 # disables joint correction
    pivot.max_force = 1000 # enables friction
...
def create_wall(poly_dim):
    body = pymunk.Body(body_type = pymunk.Body.STATIC)
    body.position = ((0, 0))
    shape = pymunk.Poly(body, poly_dim)
...
# cue stick power bars

power_bar = pygame.Surface((10, 20))
power_bar.fill(RED)

#game loop so the game stays alive
...
while run:
    ...
    if powering_up == True and game_running == True:
        ...
        for b in range(math.ceil(force / 2000)):
            ...
            (balls[-1].body.position[0] -45 + (b * 15),
    ...
    draw_text("LIVES: " + str(lives), font, WHITE, SCREEN_WIDTH - 200, SCREEN_HEIGHT + 16)


    # draw potted balls at bottom
    for i, ball in enumerate(potted_balls):
        screen.blit(ball, (10 + (i * 50), SCREEN_HEIGHT + 6))

    # check for game over
    if lives <= 0:
        draw_text("GAME OVER", large_font, WHITE, SCREEN_WIDTH / 2 - 123, SCREEN_HEIGHT / 2 - 100)
        game_running = False

    # check for if all balls potted
    if len(balls) == 1:
